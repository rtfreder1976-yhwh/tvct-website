...
Names the buyer's own organization, which gets it past a scanning facility manager and makes it findable later when they search their inbox during a vendor comparison. Highest open for a delivery email.

> **Merge-field note:** `{{contact.company_name}}` is GHL's **built-in standard** contact field — confirmed present by Todd, 2026-08-25. It does not appear in the custom-fields API response (that endpoint returns custom fields only), so an API check will look like it is missing. It is not. **Fallback if a given contact has no company on file:** `"Your cleaning quote from The Valley Clean Team"` — worth setting as the GHL fallback value so a blank company never renders as an empty gap.
