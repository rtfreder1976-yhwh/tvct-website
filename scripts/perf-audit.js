...
const CONFIG = {
  ...
  allowedImageFormats: ['.webp', '.svg', '.avif'],
    ...
    if (content.includes('application/ld+json')) {
      ...
      if (schemaMatch) {
        try {
          ...
          // Skip empty content or Astro expressions
          if (!schemaContent || !schemaContent.startsWith('{') || schemaContent.includes('JSON.stringify')) {
            // Dynamic schema or empty - skip validation
          } else {
            JSON.parse(schemaContent);
          }
        } catch (e) {
          // Only report error if it really looks like JSON but failed
    ...
    if (headMatch) {
      ...
      scriptMatches.forEach(script => {
        if (!script.includes('defer') &&
          ...
          !script.includes('type="text/partytown"')) {
