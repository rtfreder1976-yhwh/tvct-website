...
| GA4 read access + code audit | Measurement | 2026-08-31 | ga4-connection | ⏸ awaiting Todd | `scripts/ga4-report.js` (`npm run ga4`) reads the GA4 Data API via the existing `googleapis` dep, reusing the GSC service account. Blocked on Todd: enable Google Analytics Data API in Cloud, add the service account as Viewer in GA4 Property access management, put the NUMERIC property ID in `.env`. Audit found: quote flow fires **zero** analytics events (high); phone taps reach GA4 as generic `click` with UA-style category/label, not a named event; likely no key events configured. campaigns/ga4-connection/ |
