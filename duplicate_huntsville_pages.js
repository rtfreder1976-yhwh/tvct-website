...
for (const page of pages) {
    ...
    if (fs.existsSync(srcPath)) {
        ...
        // We use regex with case-sensitive matching for proper casing.
        content = content.replace(/Athens/g, 'Huntsville');
        content = content.replace(/athens/g, 'huntsville');
        content = content.replace(/Limestone/g, 'Madison');
        content = content.replace(/limestone/g, 'madison');

        // Also check if there are specific text strings we should be careful about
        // such as "East Athens" -> "East Huntsville", "West Athens" -> "West Huntsville" etc.
        // The regex replaces will handle this automatically.
        // Some areas might need minor tweaking later but this closes the primary content gap.
